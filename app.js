...
function isFollowupDue(record, today = localDateKey()) {
  return Boolean(record
    && record.followupEnabled === true
    && record.followupStatus === 'pending'
    && isValidLocalDateKey(record.followupDate)
    && record.followupDate <= today);
}

function dueFollowups(records, today = localDateKey(), dismissedIds = new Set()) {
  const isDismissed = (id) => dismissedIds instanceof Set ? dismissedIds.has(id) : dismissedIds.includes(id);
  return (Array.isArray(records) ? records : [])
    .filter((record) => isFollowupDue(record, today) && !isDismissed(record.id))
    .slice()
    .sort((left, right) => left.followupDate.localeCompare(right.followupDate)
      || String(left.createdAt || '').localeCompare(String(right.createdAt || '')));
}

function validSatisfaction(value) {
  const number = Number(value);
  return Number.isInteger(number) && number >= 1 && number <= 5 ? number : 3;
}

if (typeof module !== 'undefined' && module.exports) {
  module.exports = { localDateKey, isValidLocalDateKey, isFollowupDue, dueFollowups, validSatisfaction };
}
...
if (typeof document !== 'undefined') (function () {
  ...
  let toastTimer;
  let activeFollowupId = '';
  const sessionDismissedFollowups = new Set();
  ...
  function ensureClarificationPanel() {
    ...
    panel.hidden = true;
    panel.innerHTML = `
      <div class="clarification-badge">AI 选择先问一句</div>
      <p class="clarification-question" id="clarificationQuestion"></p>
      <label for="clarificationAnswer">你的补充</label>
      <textarea id="clarificationAnswer" maxlength="500" rows="3" placeholder="一句话回答就够了"></textarea>
      <div class="clarification-actions">
        <button class="secondary-button" id="skipClarification" type="button">先跳过，直接分析</button>
        <button class="primary-button" id="submitClarification" type="button">补充后继续 <span>→</span></button>
      </div>`;
    $('.flow-step[data-step="1"] .step-title-row').insertAdjacentElement('afterend', panel);
    return panel;
  }
  ...
  function hideClarificationPanel() {
    ...
    const step = $('.flow-step[data-step="1"]');
    if (step) step.classList.remove('is-waiting-clarification');
  }

  function renderClarificationPanel() {
    if (!state.pendingClarification) return hideClarificationPanel();
    const panel = ensureClarificationPanel();
    $('#clarificationQuestion').textContent = state.pendingClarification.question;
    $('#clarificationAnswer').value = state.clarification?.answer || '';
    panel.hidden = false;
    $('.flow-step[data-step="1"]').classList.add('is-waiting-clarification');
    $('.next-step[data-next="2"]').disabled = true;
  }

  function showClarification(request) {
    state.analysis = null;
    state.options = [];
    state.pendingClarification = {
      question: String(request?.question || '').trim().slice(0, 80),
      missing_information: String(request?.missing_information || '关键背景').trim().slice(0, 40),
    };
    state.clarification = null;
    $('#step-one-title').textContent = '在拆开选项前，我想先确认一件事。';
    $('#analysisSubtitle').textContent = `补上“${state.pendingClarification.missing_information}”，后面的分析会更贴近现实。最多只问这一句。`;
  ...
  async function submitClarification(skip = false) {
    ...
    const input = $('#clarificationAnswer');
    const typedAnswer = input.value.trim();
    if (!skip && !typedAnswer) {
      showToast('一句话回答就够了，也可以选择先跳过。');
      input.focus();
      return;
    }
    const answerForAI = skip ? '用户选择跳过补充，请基于现有信息继续分析。' : typedAnswer;
    state.clarification = { question: state.pendingClarification.question, answer: skip ? '已跳过' : typedAnswer };
    persistState();
    const submitButton = $('#submitClarification');
    const skipButton = $('#skipClarification');
    setButtonLoading(submitButton, true, '正在结合补充继续…');
    skipButton.disabled = true;
    try {
      const result = await callAI('analyze', {
        question: state.question,
        clarification: { question: state.pendingClarification.question, answer: answerForAI },
      ...
      applyAnalysis(normalizeAnalysis(analysis, state.question));
      showToast(skip ? '已跳过追问，并按现有信息继续。' : '已结合你的补充重新整理选项。');
    } catch (error) {
      applyAnalysis(normalizeAnalysis(localFallbackAnalysis(state.question), state.question));
      showToast(`AI 连接稍慢，已先按现有信息继续：${error.message}`);
    } finally {
  ...
  function initClarification() {
    ensureClarificationPanel();
    $('#submitClarification').addEventListener('click', () => submitClarification(false));
    $('#skipClarification').addEventListener('click', () => submitClarification(true));
    $('#clarificationAnswer').addEventListener('keydown', (event) => {
      if ((event.ctrlKey || event.metaKey) && event.key === 'Enter') submitClarification(false);
    });
  }
  ...
  async function analyzeQuestion() {
    ...
    setButtonLoading(button, true, '正在判断还缺什么信息…');
    try {
      const result = await callAI('analyze', { question, clarification_round: 0 });
      if (result.risk_level === 'S3' || result.risk_level === 'S4') {
        renderSafetyResponse(result.safety_message);
        $('#mirror').scrollIntoView({ behavior: 'smooth' });
        return;
      }
      if (result.next_action === 'ask' && result.clarification?.question) {
        showClarification(result.clarification);
        showToast('AI 发现一个会影响分析的关键信息。');
        return;
      }
      applyAnalysis(normalizeAnalysis(result.analysis || result, question));
      showToast('DeepSeek 判断信息已足够，并整理了选项。');
    } catch (error) {
  ...
  function initDecision() {
    ...
    const followupEnabled = $('#followupEnabled');
    const followupDate = $('#followupDate');
    const followupDatePanel = $('#followupDatePanel');
    const setDefaultFollowupDate = () => {
      const nextWeek = new Date();
      nextWeek.setDate(nextWeek.getDate() + 7);
      followupDate.value = localDateKey(nextWeek);
      updateCalendarPreview(followupDate.value);
    };
    followupDate.min = localDateKey();
    followupEnabled.checked = false;
    followupDate.value = '';
    followupDatePanel.hidden = true;
    followupEnabled.addEventListener('change', () => {
      followupDatePanel.hidden = !followupEnabled.checked;
      if (followupEnabled.checked && !followupDate.value) setDefaultFollowupDate();
      if (!followupEnabled.checked) followupDate.value = '';
    });
    $('#followupDate').addEventListener('change', (event) => {
      if (event.target.value && event.target.value < localDateKey()) {
        event.target.value = localDateKey();
        showToast('回访日期不能早于今天，已经为你调整到今天。');
      }
      updateCalendarPreview(event.target.value);
    });
    $('#saveDecisionButton').addEventListener('click', saveDecision);
    $('#newDecisionButton').addEventListener('click', resetCurrentDecision);
  }

  function updateCalendarPreview(dateValue) {
    const parts = String(dateValue || '').split('-');
    const monthNames = ['JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL', 'AUG', 'SEP', 'OCT', 'NOV', 'DEC'];
    $('#calendarDay').textContent = parts.length === 3 ? parts[2] : '--';
    $('#calendarMonth').textContent = parts.length === 3 ? monthNames[Number(parts[1]) - 1] : '---';
  }
  ...
  function saveDecision() {
    ...
    const followupEnabled = $('#followupEnabled').checked;
    const followupDate = followupEnabled ? $('#followupDate').value : '';
    if (followupEnabled && (!isValidLocalDateKey(followupDate) || followupDate < localDateKey())) {
      showToast('请选择今天或之后的回访日期。');
      $('#followupDate').focus();
      return;
    }
    const record = {
      ...
      followupEnabled,
      followupDate,
      followupStatus: followupEnabled ? 'pending' : 'none',
    ...
    $('#completionCopy').textContent = record.followupEnabled
      ? `你选择了“${record.choice}”。${record.followupDate} 之后再次打开这里时，可以轻轻回看一次。`
      : `你选择了“${record.choice}”。这次没有开启回访，记录会安静地留在你的足迹里。`;
    renderMemory();
  ...
  function renderMemory() {
    ...
    const pendingCount = state.records.filter((record) => record.followupEnabled === true && record.followupStatus === 'pending').length;
    $('#recordCount').textContent = hasRecords ? `${state.records.length} 条记录 · ${pendingCount} 个待回访` : '0 条记录 · 等待开启';
    ...
    timeline.innerHTML = state.records.map((record) => {
      ...
      const followupLabel = record.followupStatus === 'reviewed'
        ? '已完成回访'
        : record.followupEnabled === true && record.followupStatus === 'pending'
          ? '等待回访'
          : record.followupStatus === 'cancelled' ? '已取消回访' : '未开启回访';
      let followupDetails = `<p>${record.followupEnabled === true && record.followupStatus === 'pending' && record.followupDate
        ? `预计 ${escapeHTML(record.followupDate)} 回来看看`
        : followupLabel}</p>`;
      if (record.followupStatus === 'reviewed') {
        followupDetails = `<div class="reviewed-result">
          <p><span>当时预期满意度</span><strong>${validSatisfaction(record.expectedSatisfaction)} / 5</strong></p>
          <p><span>实际满意度</span><strong>${validSatisfaction(record.actualSatisfaction)} / 5</strong></p>
          <p><span>最后实际选择</span><strong>${escapeHTML(record.actualChoice)}</strong></p>
          ${record.outcomeNote ? `<p><span>后来发现</span><strong>${escapeHTML(record.outcomeNote)}</strong></p>` : ''}
        </div>`;
      }
      return `<div class="timeline-item editable-record" data-record-id="${record.id}"><span>${stamp}</span><div><small>${escapeHTML(record.card || '本地决定')} · ${followupLabel}</small><input class="record-question" value="${escapeHTML(record.question)}" readonly/><input class="record-choice" value="${escapeHTML(record.choice)}" readonly/>${followupDetails}</div><div class="record-actions"><button type="button" data-edit-record>编辑</button><button type="button" data-delete-record>删除</button></div></div>`;
    }).join('');
  }

  function showNextDueFollowup() {
    const prompt = $('#followupPrompt');
    const next = dueFollowups(state.records, localDateKey(), sessionDismissedFollowups)[0];
    if (!next) {
      activeFollowupId = '';
      prompt.hidden = true;
      return;
    }
    activeFollowupId = next.id;
    $('#followupQuestion').textContent = `“${next.question || '这次决定'}”`;
    $('#followupOriginalChoice').textContent = next.choice || '未填写';
    $('#followupExpectedSatisfaction').textContent = `${validSatisfaction(next.expectedSatisfaction)} / 5`;
    $('#followupPromptCopy').hidden = false;
    $('#followupSurvey').hidden = true;
    $('#followupFormError').hidden = true;
    prompt.hidden = false;
  }

  function dismissActiveFollowupForSession() {
    if (activeFollowupId) sessionDismissedFollowups.add(activeFollowupId);
    showNextDueFollowup();
  }

  function initFollowups() {
    $('#openFollowupSurvey').addEventListener('click', () => {
      $('#followupPromptCopy').hidden = true;
      $('#followupSurvey').hidden = false;
      $('#actualChoiceInput').value = '';
      $('#actualSatisfactionRange').value = '3';
      $('#actualSatisfactionOutput').textContent = '3 / 5';
      $('#outcomeNoteInput').value = '';
      $('#followupFormError').hidden = true;
      $('#actualChoiceInput').focus();
    });
    $('#actualSatisfactionRange').addEventListener('input', (event) => {
      $('#actualSatisfactionOutput').textContent = `${validSatisfaction(event.target.value)} / 5`;
    });
    $('#snoozeFollowup').addEventListener('click', dismissActiveFollowupForSession);
    $('#closeFollowupSurvey').addEventListener('click', dismissActiveFollowupForSession);
    $('#cancelFollowup').addEventListener('click', () => {
      const record = state.records.find((item) => item.id === activeFollowupId);
      if (!record) return showNextDueFollowup();
      record.followupStatus = 'cancelled';
      persistState();
      renderMemory();
      showToast('好的，这次决定以后不再回访。');
      showNextDueFollowup();
    });
    $('#followupSurvey').addEventListener('submit', (event) => {
      event.preventDefault();
      const actualChoice = $('#actualChoiceInput').value.trim();
      if (!actualChoice) {
        $('#followupFormError').hidden = false;
        $('#actualChoiceInput').focus();
        return;
      }
      const record = state.records.find((item) => item.id === activeFollowupId);
      if (!record || !isFollowupDue(record, localDateKey())) return showNextDueFollowup();
      record.actualChoice = actualChoice;
      record.actualSatisfaction = validSatisfaction($('#actualSatisfactionRange').value);
      record.outcomeNote = $('#outcomeNoteInput').value.trim();
      record.followupStatus = 'reviewed';
      record.reviewedAt = new Date().toISOString();
      persistState();
      renderMemory();
      showToast('这次回访已经保存，谢谢你回来看看。');
      showNextDueFollowup();
    });
  }
  ...
  function initMemory() {
    $('#memoryTimeline').addEventListener('click', (event) => {
      ...
      if (event.target.closest('[data-delete-record]')) {
        ...
        persistState(); renderMemory(); showToast('这条记录已删除。');
        if (record.id === activeFollowupId) showNextDueFollowup();
        return;
      }
  ...
  function restoreUI() {
    if (state.pendingClarification) renderClarificationPanel();
